...
\end{equation}
where $\lambda \geq 0$ is a learnable Lagrange multiplier. The penalty term increases the loss when the KL constraint is violated, discouraging the adversary from deviating too far from the cooperative policy. The multiplier $\lambda$ is updated via dual gradient ascent on the hinge-relaxed constraint, matching the primal objective:
\begin{equation}
    \lambda \leftarrow \max\!\left(0,\; \lambda + \eta_\lambda \cdot \max\!\left(0,\; D_\text{KL}(\pi_{\theta_\text{adv}} \| \pi_\text{coop}) - \delta\right)\right),
\end{equation}
which increases $\lambda$ when the constraint is violated, leaves it unchanged when the constraint is satisfied (due to the inner $\max$), and projects onto $\lambda \geq 0$ (outer $\max$). In our implementation, we parameterize $\lambda = e^{\mu}$ and optimize $\mu$ by gradient descent, which naturally enforces $\lambda > 0$.
...
\midrule
\textbf{Adversarial agent} & --- & $\mathbf{-80.2\ [-88.6, -71.9]}$ & $0.019^*$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Attack transferability (exploratory, single-seed evaluation). QMIX did not fully converge ($-167$ vs.\ MAPPO's $-65.5$), limiting the interpretability of these results.}
\label{tab:transfer}
\begin{tabular}{llr}
\toprule
Attack & Source $\to$ Target & Mean Reward \\
\midrule
Clean & MAPPO (5-seed) & $-65.5\ [-69.9, -61.1]$ \\
Clean & QMIX (1-seed) & $-167.0$ \\
\midrule
FGSM ($\epsilon{=}0.5$) & MAPPO $\to$ MAPPO (5-seed) & $-75.0\ [-79.6, -70.3]$ \\
FGSM ($\epsilon{=}0.5$) & MAPPO $\to$ QMIX (1-seed) & $-161.5$ \\
\midrule
Adversarial agent & MAPPO $\to$ MAPPO (5-seed) & $-80.2\ [-88.6, -71.9]$ \\
Adversarial agent & MAPPO $\to$ QMIX (1-seed) & $-174.8$ \\
\bottomrule
\end{tabular}
\end{table}

\Cref{tab:transfer} presents exploratory transferability results. FGSM perturbations crafted against MAPPO do not degrade QMIX's performance ($-161.5$ vs.\ clean $-167.0$). The adversarial agent has modest additional impact on QMIX ($-174.8$, a $4.7\%$ degradation) compared to a much larger effect on MAPPO ($-80.2$, a $22\%$ degradation from the 5-seed baseline).

These results are suggestive but must be interpreted with caution. QMIX's substantially worse clean performance ($-167$ vs.\ $-65.5$) reflects underconvergence of our value-decomposition implementation, consistent with prior benchmarking \citep{papoudakis2021benchmarking}. We cannot distinguish whether weak transfer reflects true algorithmic specificity or simply a poor target policy. The QMIX results use a single training seed and lack confidence intervals. We present these as a pilot study motivating future work with properly converged baselines.
...
\begin{table}[H]
\centering
\caption{Communication robustness: MAPPO (5 seeds, 95\% CI) vs.\ CommNet (single model, $\pm$ std over 100 episodes). The asymmetry in statistical basis means direct numerical comparisons should be made cautiously.}
\label{tab:comm}
\begin{tabular}{lrr}
\toprule
FGSM $\epsilon$ & MAPPO [95\% CI] & CommNet (single model) \\
\midrule
0 (clean) & $-65.5\ [-69.9, -61.1]$ & $-72.6 \pm 20.3$ \\
0.1 & $-66.3\ [-69.2, -63.5]$ & $-71.0 \pm 21.4$ \\
0.2 & $-68.8\ [-71.5, -66.1]$ & $-71.3 \pm 18.4$ \\
0.5 & $-75.0\ [-79.6, -70.3]$ & $-74.3 \pm 19.8$ \\
1.0 & $-82.2\ [-88.0, -76.4]$ & $-73.9 \pm 20.3$ \\
\bottomrule
\end{tabular}
\end{table}

\Cref{fig:comm_robustness,tab:comm} show a suggestive pattern, though CommNet is evaluated with only a single training seed and comparisons must be interpreted cautiously. MAPPO's reward degrades monotonically with $\epsilon$, reaching $-82.2$ at $\epsilon = 1.0$. CommNet's reward is relatively stable across all $\epsilon$ values ($-71$ to $-74$), suggesting that the communication channel's averaging operation may act as a low-pass filter at high noise levels. However, CommNet's \emph{clean} performance is also worse ($-72.6$ vs.\ $-65.5$), so the apparent robustness at high $\epsilon$ may partly reflect a lower performance ceiling rather than genuine resilience. Multi-seed CommNet training would be needed to confirm these observations.
...
\begin{table}[H]
\centering
\caption{Stealth attack trade-off: effectiveness vs.\ detectability. Reward is mean across 5 MAPPO seeds [95\% CI]. Achieved KL is measured empirically over evaluation episodes. TPR/FPR from the single-seed anomaly detector.}
\label{tab:stealth}
\begin{tabular}{lrrrr}
\toprule
Agent type & Reward [95\% CI] & Achieved KL & TPR & FPR \\
\midrule
Clean & $-65.5\ [-69.9, -61.1]$ & --- & $0\%$ & $0\%$ \\
Unconstrained & $-80.2\ [-88.6, -71.9]$ & --- & $98\%$ & $0\%$ \\
Stealth ($\delta = 2.0$) & $-74.4\ [-78.8, -70.1]$ & $4.78 \pm 2.63$ & $38\%$ & $0\%$ \\
Stealth ($\delta = 0.5$) & $-72.6\ [-77.3, -67.9]$ & $4.51 \pm 2.53$ & $27\%$ & $0\%$ \\
Stealth ($\delta = 0.1$) & $-72.6\ [-77.3, -67.8]$ & $4.51 \pm 2.53$ & $27\%$ & $0\%$ \\
\bottomrule
\end{tabular}
\end{table}

\Cref{fig:stealth,tab:stealth} reveal that \textbf{the KL constraint was not effectively enforced}: all three stealth variants achieve KL $\approx 4.5$, far exceeding their respective targets $\delta$. This means the Lagrange multiplier did not grow fast enough to dominate the adversarial objective. Consequently, $\delta = 0.1$ and $\delta = 0.5$ produce identical behavior---neither reached its target.

Despite the constraint failure, the stealth adversaries are still \emph{less aggressive} than the unconstrained one (reward $-72$ to $-74$ vs.\ $-80$), and correspondingly harder to detect (TPR $27$--$38\%$ vs.\ $98\%$). This suggests the KL penalty has \emph{some} regularizing effect, but a stronger enforcement mechanism (e.g., a larger initial $\lambda$ or hard KL clipping) would be needed to achieve targeted stealth levels. The false positive rate is $0\%$ across all experiments.
...
\begin{enumerate}
    \item \textbf{Behavioral attacks cause substantial damage under a stronger threat model.} A trained adversarial agent degrades team reward by $22\%$ (to $-80.2$, $p < 0.05$), while the strongest observation perturbation (FGSM $\epsilon = 1.0$) causes $25\%$ degradation ($p < 0.01$). The two attacks assume fundamentally different attacker capabilities (sensor noise vs.\ insider compromise) and require different defensive strategies.
    ...
    \item \textbf{Stealth attacks show a partial effectiveness--detectability trade-off, but the KL constraint was not effectively enforced.} All stealth variants achieved KL $\approx 4.5$, far exceeding their targets. Nevertheless, the KL penalty provided some regularization: stealth adversaries were less aggressive ($-72$ to $-74$) and harder to detect (TPR $27$--$38\%$) than the unconstrained adversary ($-80$, TPR $98\%$). Stronger enforcement mechanisms are needed.

    \item \textbf{In our setup, no tested defense substantially mitigates FGSM.} FGSM adversarial training destabilized on-policy RL training in this environment, and observation smoothing provided only marginal benefit that an adaptive attacker could compensate for. These negative results are specific to our discrete-action Simple Spread setting and PPO-based training; other environments, architectures, or training regimes may respond differently.
\end{enumerate}
